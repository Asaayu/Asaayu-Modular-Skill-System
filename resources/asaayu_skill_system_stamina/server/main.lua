...
Citizen.CreateThread(function()
    ...
    local result = exports.asaayu_skill_system:registerSkill("stamina", "Stamina", 100, 1000, 0.12)
    if not result then error("Failed to register 'stamina' skill.") end
    loaded = true
end)
...
local lastCall = {}
RegisterNetEvent("asaayu_skill_system_stamina:action", function()
    local source = source
    ...
    -- Unforunately, due to FiveM server limitations we can't check if the player is sprinting or moving fast enough, so we'll just have to trust them and just use a cooldown timer to ensure they don't get xp too often.
    ...
    -- Give the player xp.
    exports.asaayu_skill_system:addXp(source, "stamina", amount)
end)


-- Register a net event get the player's xp and level.
RegisterNetEvent("asaayu_skill_system_stamina:getLevel", function()
    local source = source
    ...
    local data = exports.asaayu_skill_system:getXp(source, "stamina")

    -- Get the max level of the skill.
    local maxLevel = exports.asaayu_skill_system:getMaxLevel("stamina")

    -- Send the data back to the client.
    TriggerClientEvent("asaayu_skill_system_stamina:getLevel", source, data[1], data[2], maxLevel)
end)