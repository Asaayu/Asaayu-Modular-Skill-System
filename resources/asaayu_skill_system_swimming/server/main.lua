...
Citizen.CreateThread(function()
    ...
    local result = exports.asaayu_skill_system:registerSkill("swimming", "Swimming", 100, 1000, 0.12)
    if not result then error("Failed to register 'swimming' skill.") end
    loaded = true
end)
...
local lastCall = {}
RegisterNetEvent("asaayu_skill_system_swimming:action", function()
    local source = source
    ...
    -- Give the player xp.
    exports.asaayu_skill_system:addXp(source, "swimming", amount)
end)


-- Register a net event get the player's xp and level.
RegisterNetEvent("asaayu_skill_system_swimming:getLevel", function()
    local source = source
    ...
    local data = exports.asaayu_skill_system:getXp(source, "swimming")

    -- Get the max level of the skill.
    local maxLevel = exports.asaayu_skill_system:getMaxLevel("swimming")

    -- Send the data back to the client.
    TriggerClientEvent("asaayu_skill_system_swimming:getLevel", source, data[1], data[2], maxLevel)
end)