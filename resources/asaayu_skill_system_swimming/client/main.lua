...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent("asaayu_skill_system_swimming:getLevel")
    ...
    while true do
        ...
        local swimming = IsPedSwimmingUnderWater(player)

        -- Check the requirements to get xp.
        if swimming and GetEntitySpeed(player) > 3.5 then

            -- Only call the function once the player has waited long enough.
            if lastCall < GetGameTimer() then
                TriggerServerEvent("asaayu_skill_system_swimming:action")
...
function handleBonus(level)
    ...
    StatSetInt(`MP0_LUNG_CAPACITY`, progress, true)

end

-- Listen for the event that returns the player's xp and level.
RegisterNetEvent("asaayu_skill_system_swimming:getLevel", function(xp, level, max)
...
RegisterNetEvent("asaayu_skill_system:xpEarned", function(skill, displayName, oldAmount, amount, newAmount, origLevel, newLevel, xpRequired)
    ...
    if skill ~= 'swimming' then return end
...
-- We never remove xp from the swimming skill, so we don't need to listen for the remove events.