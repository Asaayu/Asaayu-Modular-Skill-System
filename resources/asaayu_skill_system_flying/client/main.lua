...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent("asaayu_skill_system_flying:getLevel")

    -- Create a loop that we can use to detect when the player is flying.
    local lastCall = 0
    while true do
        ...
        local flying = IsPedInFlyingVehicle(player)

        -- Check the requirements to get xp.
        if vehicle and flying and GetPedInVehicleSeat(vehicle, -1) == player and GetEntitySpeed(vehicle) > 30 then

            -- Only call the function every minute
            if lastCall < GetGameTimer() then
                TriggerServerEvent("asaayu_skill_system_flying:action")
...
function handleBonus(level)
    ...
    StatSetInt(`MP0_FLYING_ABILITY`, progress, true)

end

-- Listen for the event that returns the player's xp and level.
RegisterNetEvent("asaayu_skill_system_flying:getLevel", function(xp, level, max)
...
RegisterNetEvent("asaayu_skill_system:xpEarned", function(skill, displayName, oldAmount, amount, newAmount, origLevel, newLevel, xpRequired)
    ...
    if skill ~= 'flying' then return end
end)
...
RegisterNetEvent("asaayu_skill_system:levelEarned", function(skill, displayName, oldAmount, amount, newAmount, origLevel, newLevel, xpRequired)
    ...
    AddTextComponentSubstringPlayerName("\nTurbulence will be easier to handle.")
...
-- We never remove xp from the flying skill, so we don't need to listen for the remove events.