...
Citizen.CreateThread(function()
    ...
    local result = exports.asaayu_skill_system:registerSkill("driving", "Driving", 100, 1000, 0.12)
    if not result then error("Failed to register 'driving' skill.") end
    loaded = true
end)
...
local lastCall = {}
RegisterNetEvent("asaayu_skill_system_driving:action", function()
    local source = source
    ...
    -- For security, lets confirm that the player is driving.
    local player = GetPlayerPed(source)
    local vehicle = GetVehiclePedIsIn(player, false)

    -- Confirm that all the requirements are met on the server side as well.
    if not vehicle or GetPedInVehicleSeat(vehicle, -1) ~= player or GetEntitySpeed(vehicle) < 10 then return end
    ...
    -- Give the player xp.
    exports.asaayu_skill_system:addXp(source, "driving", math.floor(amount))
end)


-- Register a net event get the player's xp and level.
RegisterNetEvent("asaayu_skill_system_driving:getLevel", function()
    local source = source
    ...
    local data = exports.asaayu_skill_system:getXp(source, "driving")

    -- Get the max level of the skill.
    local maxLevel = exports.asaayu_skill_system:getMaxLevel("driving")

    -- Send the data back to the client.
    TriggerClientEvent("asaayu_skill_system_driving:getLevel", source, data[1], data[2], maxLevel)
end)